...
import FlowManager from '../components/Common/Date/FlowManager';
...
export function monthDays(year, month, weekStart) {
  ...
  return Array.apply(
    null,
    Array(monthDate.daysInMonth + firstDay + lastDay)
  ).map((value, index) =>
    index + 1 <= firstDay || index >= firstDay + monthDate.daysInMonth
      ? null
      : index + 1 - firstDay
  );
}
...
export function monthDayIsDisabled(minDate, maxDate, year, month, day) {
  ...
  minDate = minDate
    ? startOfDay(minDate.setZone('UTC', { keepLocalTime: true }))
    : null;
  maxDate = maxDate
    ? startOfDay(maxDate.setZone('UTC', { keepLocalTime: true }))
    : null;
...
export function hours(step) {
  return Array.apply(null, Array(Math.ceil(24 / step))).map(
    (item, index) => index * step
  );
}

export function minutes(step) {
  return Array.apply(null, Array(Math.ceil(60 / step))).map(
    (item, index) => index * step
  );
}

export function years(current) {
  return Array.apply(null, Array(201)).map(
    (item, index) => current - 100 + index
  );
}
...
export function weekStart() {
  ...
  try {
    weekstart = require('weekstart/package.json').version
      ? require('weekstart')
      : null;
  } catch (e) {
  ...
  const firstDay = weekstart
    ? weekstart.getWeekStartByLocale(Settings.defaultLocale)
    : 1;
