          ...
          >
            <div class="col-md-4 col-sm-4">
              <div class="form-group mb-4">
                ...
                <v-combobox hide-selected></v-combobox>
                <Select2
...
export default {
  ...
  methods: {
    ...
    addVariantField() {
      let selectedVariant = Object.keys(this.variantsListObj).map((key) => {
        return this.variantsListObj[key].variant
      })
      let availableVariant = _.cloneDeep(this.clonedVariant)

      /*for (let i = 0; i < selectedVariant.length; i++) {
        let index = availableVariant.findIndex(
          (variant) => variant.id === selectedVariant[i]
        )
        if (index >= 0) {
          availableVariant.splice(index, 1)
        }
      }*/

      this.variantsListObj.push(this.variantObj)

      this.variantsListObj[
        this.variantsListObj.length - 1
    ...
    removeVariant(index) {
      ...
      /*removedVariant = this.clonedVariant.find(
        (variant) => variant.id === removedVariant.variant
      )
      console.lo*/

      /*for (let i = 0; i < this.variantsListObj.length; i++) {
        this.variantsListObj[i].variants.push(removedVariant)
      }*/
      /*let selectedVariant = Object.keys(this.variantsListObj).map((key) => {
        return this.variantsListObj[key].variant
      })
      ...
      for (let j = 0; j < this.variantsListObj.length; j++) {
        this.variantsListObj[j].variants = availableVariant
      }*/
    },
    addVariant() {
