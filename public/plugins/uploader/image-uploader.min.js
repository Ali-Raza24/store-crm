...
(function($) {
  $.fn.imageUploader = function(options) {
    ...
    plugin.init = function() {
      ...
      plugin.each(function(i, wrapper) {
        ...
        if (plugin.settings.preloaded.length) {
          ...
          for (let i = 0; i < plugin.settings.preloaded.length; i++) {$uploadedContainer.append(createImg(plugin.settings.preloaded[i].src, plugin.settings.preloaded[i].id, !0));}
        }
      });
    };
    ...
    let createImg = function(src, id) {
      ...
      } else {$container.attr('data-index', id);}
      ...
      $button.on('click', function(e) {
        ...
        $container.remove();
        if (!$container.find('.uploaded-image').length) {$container.removeClass('has-files');}
      });
      return $container;
    };
    let fileDragHover = function(e) {
      prevent(e);
      if (e.type === 'dragover') {$(this).addClass('drag-over');} else {$(this).removeClass('drag-over');}
    };
